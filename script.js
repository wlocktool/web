...
const motorolaModels = [
  ...
  { name: "Moto G86 5G", processor: "Snapdragon 695 5G", processes: { imei: true, frp: true, mdm: true } },
...
function updateUILanguage() {
  ...
  if (downloadBtn) {
    downloadBtn.innerHTML = `<i class="fas fa-download"></i> ${getText("downloadTool")}`
  }
...
function initializeApp() {
  // Cargar preferencia de idioma primero
  ...
  // Initialize server statuses - establecer estados por defecto
  ...
  if (savedServerStatuses !== null) {
    ...
    // Si no hay estados guardados, usar los valores por defecto
    serverStatuses = {
      sprint: true,
      mtk: true,
      qualcomm: true,
...
// Agregar función para mostrar el modal de descarga
function showDownloadModal() {
  // Crear el modal de descarga si no existe
    ...
    downloadModal.innerHTML = `
      <div class="modal-content">
        <div class="modal-header">
          <h2>${getText("downloadTool")}</h2>
          <span class="close" onclick="closeDownloadModal()">&times;</span>
        </div>
        <div class="download-content">
          <p>${getText("selectDownloadOption")}</p>
          <div class="download-buttons">
            <a href="https://mega.nz/folder/wwlWCKQA#gmE9wxUcMlTOkv0zfxkdzQ" target="_blank" class="download-btn mega-btn">
              <i class="fas fa-cloud-download-alt"></i> ${getText("downloadOption1")}
            </a>
            <a href="https://www.mediafire.com/file/d7vbwmxf93yglqr/WLOCK-TOOL.rar/file" target="_blank" class="download-btn mediafire-btn">
              <i class="fas fa-download"></i> ${getText("downloadOption2")}
            </a>
          </div>
          <p class="download-note">${getText("downloadNote")}</p>
        </div>
      </div>
    `
...
// Cerrar modal de descarga al hacer clic fuera
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Renderizar modelos después de la inicialización
  setTimeout(() => {
    renderModels()
    renderAdminModelsList()
    updateUILanguage()
...
function resetToDefaults() {
  ...
  // Clear localStorage for model processes
